...
namespace LCBRPG_User_Console.Types.ConsoleElements
{
    internal class HistoryLogElement : ElementLogic
    {
        ...
        private HistoryLogCache historyLogAuthority;

        public Stack<string> HistoryLogCache { get; set; } = new Stack<string>();
        public HistoryLogElement(ElementIdentities setUniqueIdentity, HistoryLogCache setHistoryLogAuthority)
    : base(setUniqueIdentity)
        {
            historyLogAuthority = setHistoryLogAuthority;

            //Subscribe to the history log authority's updates so that this element knows to update, too
            historyLogAuthority.LogAdded += OnHistoryLogAdded;
        }

        protected override string GenerateContent() => GetHistoryLog(historyLineMaxCount);
        protected virtual void OnHistoryLogAdded(object? sender, HistoryLogDisplayData e)
        {
            AddNewHistoryLog(e.Body);
        }
        ...
        private void AddNewHistoryLog(string newHistoryEntry)
        {
            ...
            List<string> newLogLines = WritelineUtilities.SplitIntoLines(newInformationalHistoryEntry, entryPrefix, entryInfix, entrySuffix);
        ...
        private string GetHistoryLog(int historyLineMaxCount) //Should display bottom to top :)
        {
            ...
            while (holdingStack.Count > 0)
            {
                ...
                var newSplitLines = WritelineUtilities.SplitIntoLines(poppedLog, entryPrefix, entryInfix, entrySuffix);
            ...
            //Add the ordered lines to the padded history log
