...
using static Little_Choice_Based_RPG.Types.Interactions.InteractDelegate.InteractionWithTargetObject;
...
: base(setName, newGenericDescriptor, newInspectDescriptor, setWeightInKG)
        ...
        private void HandleFixChoices()
        {
            if (isAudioBroken == true)
            {
                ...
                InteractionChoices.Add(new InteractionWithTargetObject(repairDelegate, "Repair - Re-calibrate the helmets longitudinal wave sensor-array.", "You fixed the helmet, yayy"));
            }

            if (isAudioBroken == false)
            {
                InteractUsingNothing breakDelegate = Break;
                InteractionChoices.Add(new InteractionWithNothing(breakDelegate, "Damage - Intentionally misalign the helmets longitudinal wave sensor-array", "its broken again oh nooo"));
            }
        }
